...
`;
  ...
  addGiftwrapCartClick(event) {
    ...
    // [Intercart] 包装 fetch 调用以支持广告像素上报
    handleAddToCartPixel(fetch(`${routes.cart_add_url}`, { ...fetchConfig(), ...{ body } }))
      .then((response) => response.text())
      .then((state) => {
        const parsedState = JSON.parse(state);
  ...
  customElements.define(
    ...
    class ProductForm extends HTMLElement {
      ...
      addCartAdd() {
        ...
        // [Intercart] 通过 handleAddToCartPixel 包装 fetch，上报添加购物车广告像素事件
        handleAddToCartPixel(fetch(`${routes?.cart_add_url}`, config))
          .then((response) => {
            return response.text();
          })
          .then((state) => {
